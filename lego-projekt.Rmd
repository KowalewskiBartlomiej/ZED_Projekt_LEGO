...
```{r najpopularniejsze_figurki, echo=T, cache=F}
inventory_minifigs %>%
...
```{r rozklad_materiałow, echo=T, cache=F}
parts %>%
...
```{r najpopularniejsze_tematyki, echo=T, cache=F}
themes %>%
...
```{r najwieksze_zestawy, echo=T, cache=F}
sets %>%
...
```{r najpopularniejsze_kolory, echo=T, cache=F}
colors %>%
...
```{r najpopularniejsze_kategorie, echo=T, cache=F}
parts %>%
...
```{r najpopularniejsze_części, echo=T, cache=F}
parts %>%
...
```{r transparentność_części, echo=T, cache=F}
parts %>%
...
```{r najbardziej_zlozone_motywy, echo=T, cache=F}
themes %>%
  rename(theme_name = name) %>%
...
```{r trend_nowe_tematyki, echo=T, cache=F}
themes %>%
  rename(theme_name = name) %>%
  merge(sets, by.x="id", by.y="theme_id") %>%
  filter(year <= 2023) %>%
...
```{r trend_srednio_czesci_zestaw, echo=T, cache=F}
sets %>%
  filter(year <= 2023) %>%
  group_by(year) %>%
...
```{r trend_figurki, echo=T, cache=F}
minifigs %>%
...
```{r trend_zestawy, echo=T, cache=F}
sets %>%
  filter(year <= 2023) %>%
  group_by(year) %>%
...
```{r trend_elementy_czesc, echo=T, cache=F}
elements %>%
